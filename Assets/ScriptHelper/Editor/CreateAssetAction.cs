...
namespace Buskit3D.ScriptHelper
{
    ...
    public class CreateAssetAction
    {
        ...
        [MenuItem("Assets/Create/Rainier MvvmLogicBehaviour C#", false, 60)]
        public static void MvvmLogicBehavior()
        {
            //参数为传递给CreateEventCSScriptAsset类action方法的参数  
            ProjectWindowUtil.StartNameEditingIfProjectWindowExists(0,
               ScriptableObject.CreateInstance<CreateEventCSScriptAsset>(),
               GetSelectPathOrFallback() + "/NewMvvmLogicBehaviour.cs", EditorGUIUtility.FindTexture("cs Script Icon"),
               EditorScriptsConfig.Instance.TemplatePath + "MvvmLogicBehaviour.cs.txt");
        }

        [MenuItem("Assets/Create/Rainier Index Html", false, 60)]
        public static void CreateIndex()
        {
            //参数为传递给CreateEventCSScriptAsset类action方法的参数  
            ProjectWindowUtil.StartNameEditingIfProjectWindowExists(0,
               ScriptableObject.CreateInstance<CreateEventCSScriptAsset>(),
               GetSelectPathOrFallback() + "/index.html", EditorGUIUtility.FindTexture("html Script Icon"),
               EditorScriptsConfig.Instance.TemplatePath + "Index.html.txt");
        }
    ...
    class CreateEventCSScriptAsset : EndNameEditAction
    {
        ...
        internal static UnityEngine.Object CreateScriptAssetFromTemplate(string pathName, string resourceFile)
        {
            ...
            text = Regex.Replace(text, "#NowTime#", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
            text = Regex.Replace(text, "#ReleasePathName#", EditorScriptsConfig.Instance.ReleasePathName);
